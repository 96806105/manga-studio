...
const Dashboard: React.FC = () => {
  ...
  if (projects.length === 0) return <div className="h-full flex items-center justify-center"><div className="text-center"><Video size={48} className="mx-auto mb-4 opacity-30" /><p className="text-lg">No projects yet</p><p className="text-sm mt-1">Create your first AI manga/drama project</p></div></div>;
  return <div className="h-full p-6 overflow-y-auto"><div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold text-white">My Projects</h1><button onClick={() => { const n = prompt('Project name:'); if (n) { setSelectedProjectId(n); navigate(`/project/${n}`); } }} className="flex items-center gap-2 px-4 py-2 bg-studio-600 hover:bg-studio-700 text-white rounded-lg text-sm"><Plus size={16} /> New</button></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{projects.map(p => (<div key={p.id} onClick={() => { setSelectedProjectId(p.id); navigate(`/project/${p.id}`); }} className="glass glass-hover rounded-xl p-4 cursor-pointer group transition-all"><div className="flex items-start justify-between mb-3"><div className="w-10 h-10 rounded-lg bg-gradient-to-br from-studio-600 to-manga-accent2 flex items-center justify-center"><Video size={20} className="text-white" /></div><span className={`text-xs px-2 py-1 rounded-full text-white ${statusColors[p.status] || 'bg-gray-600'}`}>{p.status}</span></div><h3 className="text-white font-medium text-sm mb-1 truncate">{p.name}</h3><p className="text-gray-500 text-xs truncate">{p.description}</p><div className="flex items-center gap-2 mt-3"><button onClick={(e) => { e.stopPropagation(); if (confirm('Delete?')) { api.deleteProject(p.id).catch(alert); } }} className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-400 transition-all"><Trash2 size={14} /></button></div></div>))}</div></div>;
};
