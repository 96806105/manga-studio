  ...
  return (
    <div>
      <div className="flex items-center justify-between p-3 border-b border-manga-border"><span className="text-sm text-gray-400">{scenes.length} scenes</span>
        <button onClick={handleGenerateAll} disabled={generating} className="flex items-center gap-1 px-2 py-1 text-xs bg-manga-accent/20 text-manga-accent rounded hover:bg-manga-accent/30 disabled:opacity-50"><Film size={12} /> All</button>
      </div>
      <div className="divide-y divide-manga-border">
        {scenes.map(scene => (
          <button key={scene.id} onClick={() => onSelectScene(scene.id)} className={clsx('w-full text-left px-3 py-3 hover:bg-white/5 transition-colors', selectedSceneId === scene.id ? 'bg-studio-900/30 border-l-2 border-studio-500' : 'border-l-2 border-transparent')}>
            <div className="flex items-center gap-2 mb-1"><span className="text-xs bg-studio-600 text-white px-2 py-0.5 rounded">Scene {scene.scene_index + 1}</span></div>
