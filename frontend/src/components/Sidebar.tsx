    ...
    <aside className="w-56 glass border-r border-manga-border flex flex-col h-full">
      <div className="p-4 border-b border-manga-border">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-white font-semibold text-base">漫剧工作台</span>
        </div>
        <button onClick={() => { const n = prompt('项目名称:'); if (n) { onSelectProject(n); navigate(`/project/${n}`); } }} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-studio-600 hover:bg-studio-700 text-white rounded-lg text-sm font-medium transition-colors"><Plus size={16} /> 新建</button>
      </div>
      <div className="flex-1 overflow-y-auto py-2">
        <div className="px-3 mb-1"><span className="text-xs text-gray-500 uppercase tracking-wider font-medium">项目</span></div>
        {projects.map(p => (
      ...
      <div className="p-3 border-t border-manga-border">
        <button onClick={() => navigate('/dashboard')} className={clsx('w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors', isActive('/dashboard') ? 'bg-studio-900/50 text-studio-300' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200')}><LayoutDashboard size={14} /> 首页</button>
        <button onClick={() => navigate('/settings')} className={clsx('w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors mt-1', isActive('/settings') ? 'bg-studio-900/50 text-studio-300' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200')}><Settings size={14} /> 设置</button>
      </div>
    </aside>
  );
};
